...
public class UserAccountController {
    ...
    public HttpResult<IPage<AutoAccount>> selctAccountAll(Long id, Integer pageNo, Integer pageSize, String phone, HttpServletRequest request, HttpServletResponse response) {
        ...
        Assert.isTrue(pageSize != null ,"条数必填");
    ...
    public HttpResult updateUser(@RequestBody AutoUser autoUser, HttpServletRequest request, HttpServletResponse response) {
        log.info("updateUser{}", JSONObject.toJSONString(autoUser));
        Assert.isTrue(autoUser.getPhone() != null ,"phone必填");
        Assert.isTrue(autoUser.getPassword() != null ,"密码必填");
        Assert.isTrue(autoUser.getLongitude() != null ,"经度必填");
        Assert.isTrue(autoUser.getLatitude() != null ,"维度必填");
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(name = "createPhone", value = "管理员手机号",required = false),
    })
    public HttpResult<IPage<AutoUser>> selctUserAll(Long id, Integer pageNo, Integer pageSize, String phone,String createPhone, HttpServletRequest request, HttpServletResponse response) {
        log.info("selctUserAll:id:{},pageNo:{},pageSize:{},phone:{}",id,pageNo,pageSize,phone);
        Assert.isTrue(id != null ,"管理员id必填");
        Assert.isTrue(pageNo != null ,"页码必填");
        Assert.isTrue(pageSize != null ,"条数必填");
        return userAccountService.selctUserAll(id,pageNo,pageSize,phone,createPhone);
    }
    ...
    @ApiOperation("用户打卡记录")
    @PostMapping("/getUserClockRecords")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "用户id",required = true),
            @ApiImplicitParam(name = "pageNo", value = "页码",required = true),
            @ApiImplicitParam(name = "pageSize", value = "条数",required = true),
            @ApiImplicitParam(name = "phone", value = "手机号",required = false),
            @ApiImplicitParam(name = "createPhone", value = "创建人手机号",required = false),
    })
    public HttpResult<IPage<UserAccountRecords>> getUserClockRecords(Long id, Integer pageNo, Integer pageSize, String phone,String createPhone, HttpServletRequest request, HttpServletResponse response) {
        Assert.isTrue(id != null ,"用户id必填");
        Assert.isTrue(pageNo != null ,"页码必填");
        Assert.isTrue(pageSize != null ,"条数必填");
        return userAccountService.getUserClockRecords(id,pageNo,pageSize,phone,createPhone);
    }


    @ApiOperation("主账户添加")
    @PostMapping("/insertMainAccount")
    public HttpResult insertMainAccount(@RequestBody AutoAccount account, HttpServletRequest request, HttpServletResponse response) {
