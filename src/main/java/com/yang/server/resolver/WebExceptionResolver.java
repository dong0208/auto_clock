package com.yang.server.resolver;

import com.yang.server.ex.XxlSsoException;
...
public class WebExceptionResolver implements HandlerExceptionResolver {
	...
	public ModelAndView resolveException(HttpServletRequest request,
		...
		// error result
		HttpResult<String> errorResult;
		if (ex instanceof XxlSsoException) {
			errorResult = HttpResult.failure(ex.getMessage());
		} else {
			errorResult = HttpResult.failure(ex.toString().replaceAll("\n", "<br/>"));
		}
		...
		if (isJson) {
			try {
				...
				response.getWriter().print("{\"code\":" + errorResult.getCode() + ", \"msg\":\"" + errorResult.getMessage() + "\"}");
			} catch (IOException e) {
