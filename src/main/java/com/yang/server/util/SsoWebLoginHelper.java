...
public class SsoWebLoginHelper {
    ...
    public static XxlSsoUser loginCheck(HttpServletRequest request, HttpServletResponse response){
        ...
        if (StringUtils.isEmpty(cookieSessionId)) {
            cookieSessionId = request.getHeader("sessionId");
            //去掉异常抛出
//            Assert.isTrue(!StringUtils.isEmpty(cookieSessionId), "登录失效,请求头中获取sessionId失败，请重新登陆！");
        }
        //cookie中没有，从header中获取
        if (StringUtils.isEmpty(cookieSessionId)) {
            cookieSessionId = request.getHeader("sessionid");
...
//            Assert.isTrue(!StringUtils.isEmpty(cookieSessionId), "登录失效,请求头中获取sessionId失败，请重新登陆！");
        ...
        //cookie中没有，从header中获取
        if (StringUtils.isEmpty(cookieSessionId)) {
            cookieSessionId = request.getHeader("cookie");
            //去掉异常抛出
//            Assert.isTrue(!StringUtils.isEmpty(cookieSessionId), "登录失效,请求头中获取sessionId失败，请重新登陆！");
        }
        //cookie中没有，从header中获取
