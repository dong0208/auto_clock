...
public class UserClockZhiTask {
    ...
    public void clockGong() {
        try {
            ...
            log.info("----------------------职校家园上午8点打卡任务开始------------------------:::" + Thread.currentThread().getName());
            ...
            for (AutoAccount account : accountList) {
                ...
                for (AutoUser autoUser : autoUsers) {
                    ...
                    if (account.getGongClockDays() - day <= 0 && account.getType() == 1) {
                        log.info("clockGong下级管理员职校家园打卡剩余天数小于1天打卡结束::account = {}", JSONObject.toJSONString(account));
                        break;
                    }
    ...
    public void clockGongPm() {
        try {
            ...
            log.info("----------------------职校家园下午6点打卡任务开始------------------------:::" + Thread.currentThread().getName());
            long startTime = System.currentTimeMillis();
            List<AutoAccount> accountList = autoAccountMapper.selectList(new QueryWrapper<AutoAccount>().lambda().select(AutoAccount::getId)
                    .eq(AutoAccount::getAccountStatus, 0)
                    .eq(AutoAccount::getIsDelete, 0)
            ...
            for (AutoAccount account : accountList) {
                ...
                for (AutoUser autoUser : autoUsers) {
                    if (autoUser.getClockDays() <= 0 || !autoUser.isEnable() || StringUtils.isEmpty(autoUser.getWeeks()) || autoUser.getClockPm() == 0) {
                        continue;
                    }
                    //判断今天周几
                    LocalDateTime localDateTime = LocalDateTime.now();
                    ...
                    if (account.getGongClockDays() - day <= 0 && account.getType() == 1) {
                        log.info("clockGongPm下级管理员职校家园打卡剩余天数小于1天打卡结束::account = {}", JSONObject.toJSONString(account));
                        break;
                    }
