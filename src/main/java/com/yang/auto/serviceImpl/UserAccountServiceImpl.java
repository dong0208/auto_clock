...
public class UserAccountServiceImpl implements UserAccountService {
    ...
    public HttpResult insertAccount(AutoAccount account) {
        //查询创建人信息
        AutoAccount autoAccount = autoAccountMapper.selectOne(new QueryWrapper<AutoAccount>().lambda().eq(AutoAccount::getId, account.getId()).eq(AutoAccount::getType, 0));
        if (autoAccount == null) {
    ...
    public HttpResult updateAccount(AutoAccount account) {
        ...
        if (account.getZhiClockDays() != null) {
